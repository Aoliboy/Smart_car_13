...
float error_1=0;
float error_2=0;
float spdL_aaa=0.5, spdR_aaa=0.5;
...
//    PIDCTRL_ErrUpdate(&spdL_Pid, spdL_goal - spdL_now);
//    spdL_Output += PIDCTRL_DeltaPIGain(&spdL_Pid);
//    PIDCTRL_ErrUpdate(&spdR_Pid, spdR_goal - spdR_now);
//    spdR_Output += PIDCTRL_DeltaPIGain(&spdR_Pid);
...
float CTRL_SpdFix(float x)
{
    if(x > 0.15)
    {
        return (0.3558*x*x*x - 0.2983*x*x + 0.6765*x + 0.0014);
    }
    else if(x < -0.15)
    {
        return  (-0.7871*x*x*x - 0.2726*x*x - 0.5475*x - 0.0033);
    }
    return 0;
}
